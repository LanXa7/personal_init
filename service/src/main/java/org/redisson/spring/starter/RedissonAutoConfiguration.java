...
public class RedissonAutoConfiguration {
    ...
    private boolean hasConnectionDetails() {
        try {
            Class.forName("org.springframework.boot.autoconfigure.data.redis.DataRedisConnectionDetails");
            return true;
    ...
    public RedissonClient redisson() throws IOException {
        ...
        Object timeoutValue = ReflectionUtils.invokeMethod(timeoutMethod, dataRedisProperties);
        String prefix = getPrefix();

        String username = null;
        int database = dataRedisProperties.getDatabase();
        ...
        if (connectTimeoutMethod != null) {
            Object connectTimeoutValue = ReflectionUtils.invokeMethod(connectTimeoutMethod, dataRedisProperties);
        ...
        if (clientNameMethod != null) {
            clientName = (String) ReflectionUtils.invokeMethod(clientNameMethod, dataRedisProperties);
        }

        if (usernameMethod != null) {
            username = (String) ReflectionUtils.invokeMethod(usernameMethod, dataRedisProperties);
        }
        ...
        if (redissonProperties.getConfig() != null) {
        ...
        } else if (dataRedisProperties.getSentinel() != null || isSentinel) {
            String[] nodes = {};
            String sentinelMaster = null;

            if (dataRedisProperties.getSentinel() != null) {
                Method nodesMethod = ReflectionUtils.findMethod(Sentinel.class, "getNodes");
                Object nodesValue = ReflectionUtils.invokeMethod(nodesMethod, dataRedisProperties.getSentinel());
                if (nodesValue instanceof String) {
                    nodes = convert(prefix, Arrays.asList(((String) nodesValue).split(",")));
                } else {
                    nodes = convert(prefix, (List<String>) nodesValue);
                }
                sentinelMaster = dataRedisProperties.getSentinel().getMaster();
            }
            ...
            initSSL(c);
        } else if ((clusterMethod != null && ReflectionUtils.invokeMethod(clusterMethod, dataRedisProperties) != null)
                    || isCluster) {

            String[] nodes = {};
            if (clusterMethod != null && ReflectionUtils.invokeMethod(clusterMethod, dataRedisProperties) != null) {
                Object clusterObject = ReflectionUtils.invokeMethod(clusterMethod, dataRedisProperties);
            ...
            String singleAddr = prefix + dataRedisProperties.getHost() + ":" + dataRedisProperties.getPort();
    ...
    private void initSSL(BaseConfig<?> config) {
        ...
        DataRedisProperties.Ssl ssl = dataRedisProperties.getSsl();
    ...
    private String getPrefix() {
        ...
        Method getSSLMethod = ReflectionUtils.findMethod(DataRedisProperties.class, "getSsl");
        if (isSSLMethod != null) {
            if ((Boolean) ReflectionUtils.invokeMethod(isSSLMethod, dataRedisProperties)) {
                prefix = RedisURI.REDIS_SSL_PROTOCOL;
            }
        } else if (getSSLMethod != null) {
            Object ss = ReflectionUtils.invokeMethod(getSSLMethod, dataRedisProperties);
            if (ss != null) {
